...
class DocxBuilder:
    ...
    def construir(
        ...
        try:
            ...
            # ── Filtra capítulos inválidos antes de montar ───────────────────
            TITULOS_INVALIDOS = {
                "", "capítulo", "capitulo", "cap",
                "capítulo 1", "capítulo 2", "capítulo 3",
                "capitulo 1", "capitulo 2", "capitulo 3",
            }
            caps_validos = []
            for cap in capitulos:
                t_lower = cap.get("titulo", "").strip().lower()
                secoes  = cap.get("secoes", [])
                chars   = sum(len(s.get("conteudo","")) for s in secoes)
                if t_lower in TITULOS_INVALIDOS:
                    import logging; logging.getLogger(__name__).warning(
                        f"DOCX: ignorando placeholder '{t_lower}'")
                    continue
                if not secoes or chars < 100:
                    import logging; logging.getLogger(__name__).warning(
                        f"DOCX: ignorando capítulo sem conteúdo '{t_lower}'")
                    continue
                caps_validos.append(cap)

            if not caps_validos:
                raise ValueError("Nenhum capítulo válido para montar o DOCX")

            # ── Sumário apenas com capítulos válidos ──────────────────────────
            self._adicionar_sumario(doc, caps_validos)

            # ── Capítulos ─────────────────────────────────────────────────────
            for cap_dados in caps_validos:
    ...
    def _adicionar_sumario(self, doc: Document, capitulos: list):
        ...
        for cap in capitulos:
            ...
            p = doc.add_paragraph()
            run = p.add_run(f"{num:02d}.  {titulo}")
            run.font.name      = "Arial"
            run.font.size      = Pt(11)
    ...
    def _adicionar_capitulo(self, doc: Document, dados: dict):
        ...
        p = doc.add_paragraph()
        run = p.add_run(f"CAPÍTULO {numero:02d}")
        run.font.name      = "Arial"
        run.font.size      = Pt(9)
        run.font.bold      = True
        run.font.color.rgb = AZUL_MEDIO
